	...
	<el-container>
		...
		<el-main class="nopadding">
			<scTable ref="table" :apiObj="apiObj" row-key="id" border stripe>
				...
				<el-table-column label="所属分类" prop="download_cate.name" width="120"></el-table-column>
				<el-table-column label="标题" prop="name" show-overflow-tooltip min-width="300"></el-table-column>
				...
				<el-table-column label="跳转链接" prop="url" show-overflow-tooltip min-width="200"></el-table-column>
				...
				<el-table-column label="链接类型" prop="is_local" width="80">
					<template #default="scope">
						<el-tag v-if="scope.row.is_local==10" type="success">本地</el-tag>
						<el-tag v-if="scope.row.is_local==20" type="danger">外部</el-tag>
					</template>
				</el-table-column>
...
export default {
	...
	data() {
		return {
			...
			apiObj: this.$API.website.download.list,
			selection: [],
			search: {}
		}
	},
	methods: {
		...
		async del(row) {
			...
			var res = await this.$API.website.download.delete.post(reqData)
