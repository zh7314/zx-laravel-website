			...
			<scTable ref="table" :apiObj="apiObj" row-key="id" border stripe>
				...
				<el-table-column label="图片" prop="pic_path" width="100">
					<template #default="scope">
						<el-popover>
							<template #reference>
								<el-avatar :src="scope.row.pic_path" shape="square"/>
							</template>
							<template #default>
								<el-image style="width: 100%; height: 100%" :src="scope.row.pic_path"/>
							</template>
						</el-popover>
					</template>
				</el-table-column>
				<el-table-column label="显示开始时间" prop="start_time" width="150"></el-table-column>
				<el-table-column label="显示结束时间" prop="end_time" width="150"></el-table-column>
...
export default {
	...
	methods: {
		...
		async del(row) {
			...
			var res = await this.$API.website.banner.delete.post(reqData)
