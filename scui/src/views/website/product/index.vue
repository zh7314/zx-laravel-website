	...
	<el-container>
		<el-header>
			...
			<div class="right-panel">
				<div class="right-panel-search">
					<el-cascader v-model="search.product_cate_id" :options="options" :props="props"
		...
		<el-main class="nopadding">
			<scTable ref="table" :apiObj="apiObj" row-key="id" border stripe>
				...
				<el-table-column label="浏览数" prop="view_count" width="150"></el-table-column>
...
export default {
	...
	data() {
		return {
			...
			apiObj: this.$API.website.product.list,
			selection: [],
			search: {},
			//所需数据选项
			options: [],
	...
	methods: {
		...
		async getProductCate() {
			var res = await this.$API.website.productCate.getTree.get();
		...
		async del(row) {
			...
			var res = await this.$API.website.product.delete.post(reqData)
